...
void CVisuals::DrawDebugInfo(CTFPlayer* pLocal)
{
	if (Vars::Debug::Info.Value)
	{
		...
		if (pWeapon)
		{
			...
			H::Draw.StringOutlined(fFont, x, y += nTall, Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value, ALIGN_TOPLEFT, SDK::WeaponDoesNotUseAmmo(pWeapon) ? "Ammo: not used" : std::format("Ammo: {} ({} {})", pWeapon->HasAmmo(), pWeapon->m_iClip1(), pWeapon->GetWeaponInfo() ? pWeapon->GetWeaponInfo()->iMaxClip1 : -1).c_str());Add commentMore actions
